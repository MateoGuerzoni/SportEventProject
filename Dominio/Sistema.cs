...
namespace Dominio
{
    public class Sistema
    {
        ...
        //private List<Periodista> _periodistas { get; set; }
        ...
        //Metodos para retornar las listas
        ...
        public List<Reseña> GetReseñaConRoja()
        {
            List<Reseña> ListaPeriodista = new List<Reseña>();

            foreach (Reseña r in _reseñas)
            {
                if (PartidoTuvoRoja(r.Partido))
                {
                  ListaPeriodista.Add(r);
                }
                

            }
            ListaPeriodista.Sort();
            return ListaPeriodista;
        }

        //Obtengo el Pais por el nombre
        ...
        //Obtengo el partido por id
        public Partido GetPartidoById(int id)
        {
            foreach(Partido p in this._partidos)
            {
                if(p.Id == id)
                {
                    return p;
        ...
        //obtengo la reseña por id de periodista
        public Reseña GetReseñaByPeriodista(int id)
        {
            foreach(Reseña r in this._reseñas)
            {
        ...
        //devuelvo una lista de periodistas ordenados
        public List<Periodista> DevolverPeriodistasOrdenados()
        {
            List<Periodista> p = new List<Periodista>();
            foreach(Usuario u in _usuarios)
            {
                if(u.GetType().Name == "Periodista")
                {
                    p.Add((Periodista)u);
                }
            }
        ...
        //Alta de las Selecciones
        public void AltaSeleccion(Seleccion unaSeleccion)
        {

            unaSeleccion.EsValido();
            GetJugadorSeleccion(unaSeleccion.Pais);
            _selecciones.Add(unaSeleccion);

        }
        
        //Precarga de Selecciones 
        ...
        //Metodo que introduciendole un pais nos devuelve una lista de los jugadores para esa seleccion.
        public List<Jugador> GetJugadorSeleccion(Pais pa)
        {

            List<Jugador> _ret = new List<Jugador>();
            foreach (Jugador j in _jugadores)
            {
                if (j.Pais.Nombre.Equals(pa.Nombre))
                {
                    _ret.Add(j);
                }
            }
        ...
        //obtengo la reseña por email
        public List<Reseña> GetReseñaPorEmail(string unEmail)
        {
            List<Reseña> retorno = new List<Reseña>();

            foreach(Reseña r in _reseñas)
            {
                if(r.Periodista.Email == unEmail)
                {
                    retorno.Add(r);
                }
        ...
        //Obtener la seleccion por el nombre
        private Seleccion ObtenerSeleccion(string nombre)
        {
            foreach (Seleccion unaSeleccion in this._selecciones)
            {
                if (unaSeleccion.Pais.Nombre.ToUpper() == nombre.ToUpper())
                {
                    return unaSeleccion;
                }
            }
        ...
        //Busco el jugador por su id y lo retorno
        private Jugador ObtenerJugadorPorId(int unId)
        {
            foreach(Jugador unJugador in _jugadores){

                if(unJugador.Id == unId)
                {
                    return unJugador;
                }
        ...
        //obtengo jugador por un pais
        public Jugador ObtenerJugador(string pais)
        {
            foreach (Jugador unJugador in _jugadores)
            {

                if (unJugador.Pais.Nombre == pais)
                {
                    return unJugador;
                }
        ...
        //Modificar el Monto de referencia para categorizar a jugador
        public void CambiarElMontoDeReferenciaDelJugador(double unValor)
        {
            if (unValor > 0)
            {
                Jugador.MontoReferencia = unValor;
            }
            else
            {
                throw new Exception("El Monto De Referencia no puede ser menor a 0");
            }


        }

        //Obtengo los partidos que realizó el jugador mediante ID
        public List<Partido> PartidoIdJugador(int unId)
        {
            List<Partido> PartidosJugados = new List<Partido>();


            Jugador EncontrarJugador = ObtenerJugadorPorId(unId);
            ...
            foreach(Partido unPartido in _partidos)
            {
                ...
                {
                    if (unaSeleccion.Pais.Nombre.Equals(EncontrarJugador.Pais.Nombre))
                    {
                        PartidosJugados.Add(unPartido);
                    }
                }
            }
        ...
        //Se muestra todos los Jugadores Expulsados
        public List<Jugador> MostrarJugadoresExpulsados()
        {
            List<Jugador> JugadoresExpulsados = new List<Jugador>();
            foreach(Partido unPartido in this._partidos)
            {
                foreach(Incidencia unaIncidencia in unPartido.DevolverIncidencias())
                {
                    if (unaIncidencia.TipoIncidencia.Equals("Roja"))
                    {
                        if (!JugadoresExpulsados.Contains(unaIncidencia.Jugador))
                        {
                            
                            JugadoresExpulsados.Add(unaIncidencia.Jugador);
                        }
        ...
        //ordeno las selecciones
        public List<Seleccion> OrdenarSelecciones()
        {
            List<Seleccion> Selecciones = new List<Seleccion>();
            foreach (Seleccion s in this._selecciones)
            {
                Selecciones.Add(s);
        ...
        //Se obtiene los partidos con más goles que una seleccion estuvo involucrada
        public List<Partido> ObtenerListaPartidos(string unPais)
        {
            ...
            foreach (Partido unPartido in _partidos)
            {
                foreach(Seleccion unaSeleccion in unPartido.DevolverSeleccionesVs())
                {
                    if (unaSeleccion.Pais.Nombre.Equals(unPais))
                    {
            ...
            List<Partido> PartidoConMayorGoleada = DevolverListaPartidos(ListaPartidos);
            
            return PartidoConMayorGoleada;
        }

        //devuelvo la lista de goles por partido
        public List<Partido> DevolverListaPartidos(List<Partido> unaListaPartidos)
        {
            List<Partido> ListaGoleada = new List<Partido>();
            int record = 0;
            foreach(Partido unPartido in unaListaPartidos){
                int golesPorPartido = GolesPorPartido(unPartido);
                if (golesPorPartido > record)
                {
                    record = golesPorPartido;
                    ListaGoleada.Clear();
                    ListaGoleada.Add(unPartido);
                }else if (golesPorPartido.Equals(record))
                {
                    ListaGoleada.Add(unPartido);
                }
            }
        ...
        //Se muestra todos los jugadores que al mennos hicieron un Gol
        public List<Jugador> MostrarGoleadores()
        {
            List<Jugador> Goleadores = new List<Jugador>();
            foreach (Partido unPartido in this._partidos)
            {
                foreach (Incidencia unaIncidencia in unPartido.DevolverIncidencias())
                {
                    ...
                    {
                        if (!Goleadores.Contains(unaIncidencia.Jugador))
                        {

                            Goleadores.Add(unaIncidencia.Jugador);
                        }
        ...
        //Se calculan la cantidad de goles de un partido
        public int GolesPorPartido(Partido unPartido)
        {
            int goles = 0;
            foreach(Partido Partido in _partidos)
            {
                if(Partido.Id == unPartido.Id)
                {
                    foreach(Incidencia unaIncidencia in Partido.DevolverIncidencias())
                    { if(unaIncidencia.TipoIncidencia.Equals("Gol"))
                        {
                            goles++;
                        }
        ...
        //obtengo la lista y la devuelvo de la selecicon con mas goles
        public List<Seleccion> ObtenerSeleccionGoleadora()
        {
            ...
            {
                int goles = CantGolesSeleccion(s);
                if (goles > record)
                {
                    record = goles;
                    SeleccionGoleadora.Clear();
                    SeleccionGoleadora.Add(s);
                }else if (goles == record)
                {
                    SeleccionGoleadora.Add(s);
                }
            }
        ...
        //cuento los goles de una seleccion
        public int CantGolesSeleccion(Seleccion unaSeleccion)
        {
            int goles = 0;
            foreach (Partido Partido in _partidos)
            {
                if (Partido.DevolverSeleccionesVs().Contains(unaSeleccion))
                {
                    foreach (Incidencia unaIncidencia in Partido.DevolverIncidencias())
                    {
                        if (unaIncidencia.TipoIncidencia.Equals("Gol") && unaIncidencia.Jugador.Pais.Nombre == unaSeleccion.Pais.Nombre)
                        {
                            goles++;
                        }
                    }
        ...
        //Muestra Partidos en estado No finalizado
        public List<Partido> MostrarEstadoPartido()
        {
            List<Partido> retorno = new List<Partido>();
            foreach (Partido unPartido in this._partidos)
            {
                if (unPartido.Estado == "No finalizado")
                {
                    retorno.Add(unPartido);

                }
                if(unPartido.Estado == "Finalizado")
                {
                    retorno.Add(unPartido);
                }
        ...
        public string FinalizarPartido(int id)
        {
            foreach(Partido unPartido in this._partidos)
            {
                if(unPartido.Id == id && unPartido.Estado == "No finalizado")
                {
                    unPartido.SetPartido("Finalizado");
                    return unPartido.FinalizarElPartido();
                          
                }
            }
            throw new Exception("No hay partidos pendientes");
        }

        //muestro la lista de partidos finalizados mediante un id
        public List<Partido> MostrarFinalizado(int id)
        {
            List<Partido> finalizados = new List<Partido>();
            foreach(Partido unP in this._partidos)
            {
                if(unP.Id == id && unP.Estado == "Finalizado")
                {
                    finalizados.Add(unP);
                }
            }
        ...
        //Busco partido segun 2 fechas
        public List<Partido> BuscarPartidoFecha(string dato1, string dato2)
        {
            DateTime fecha1 = DateTime.Parse(dato1);
            DateTime fecha2 = DateTime.Parse(dato2);
            List<Partido> entreFechas = new List<Partido>();
            foreach(Partido unp in this._partidos)
            {
                if(unp.Fecha >= fecha1 && unp.Fecha<= fecha2)
                {
                    entreFechas.Add(unp);
                }
            }
        ...
        //busco las incidencias de un partido que me pasan por parametros
        public List<Incidencia> IncidenciaPorPartido(int id)
        {
            List<Incidencia> incidencias = new List<Incidencia>();

            foreach(Partido p in this._partidos)
            {
                foreach (Incidencia unaIncidencia in p.DevolverIncidencias())
                {
                    if (p.Id == id)
                    {
        ...
        //devuelvo una lista con las reseñas en las que hubieron tarjetas rojas
        public List<Reseña> ResConTarjetaRoja(string email)
        {
            List<Reseña> ListaPeriodista = new List<Reseña>();

            foreach(Reseña r in _reseñas)
            {
                if(r.Periodista.Email == email)
                {
                    if (PartidoTuvoRoja(r.Partido))
                    {
                        ListaPeriodista.Add(r);
        ...
        //veo si el partido que me pasan por parametros tuvo tarjeta roja
        public bool PartidoTuvoRoja(Partido unPartido)
        {
            foreach (Partido Partido in _partidos)
            {
                if (Partido.Id == unPartido.Id)
        ...
        //Se ordena la lista de Expulsados
        public List<Jugador> OrdenarListaExpulsados()
        {
            List<Jugador> JugadoresExpulsados = MostrarJugadoresExpulsados();
            JugadoresExpulsados.Sort();
            return JugadoresExpulsados;
        }

        //Alta de Periodista
        public void AltaPeriodista(Periodista unPeriodista)
        {
            try
            {
                unPeriodista.EsValido();
                if (!_usuarios.Contains(unPeriodista))
                {
                    _usuarios.Add(unPeriodista);
                ...
                {
                    throw new Exception("El Email se encuentra en uso");
                }
            }catch (Exception e)
            {
        ...
        public void AltaOperador(Operador unOperador)
        {
           try
            {
                unOperador.EsValido();
                if (!_usuarios.Contains(unOperador))
                {
                    _usuarios.Add(unOperador);
        ...
        //Alta de Reseña del periodista
        public void AltaReseña(Reseña unaReseña)
        {
            try
            {
                unaReseña.EsValido();
                _reseñas.Add(unaReseña);
        ...
        //
        public bool ExisteReseña(Reseña unaReseña)
        {
            foreach(Reseña r in _reseñas)
            {
                if(r.Periodista.Email == unaReseña.Periodista.Email && r.Partido.Id == unaReseña.Partido.Id)
                {
                    return false;
                }
            }
        ...
        //Se crea el periodista mediante datos del usuario
        public void AgregarPeriodista(string nombreP,string apellidoP, string emailP, string passwordP)
        {
           
            
            Periodista unPeriodista = new Periodista(nombreP, apellidoP, emailP, passwordP);
            AltaPeriodista(unPeriodista);
            
            

        }

        //Se agrega el periodista registrado
        public void AgregarP(Periodista p)
        {
            try
            {
                p.EsValido();
                if (instancia.Register(p.Email))
                {
                    this._usuarios.Add(p);
                }
                else
                {
                    throw new Exception("El Email se encuentra en uso");
        ...
        //agrego la reseña que me pasan por parametros
        public void AgregarReseña(Reseña unaReseña)
        {
            try
            {
        ...
        //Periodista precargado
        public void PrecargaPeriodista()
        {
            Periodista p1 = new Periodista("Aldo","Moreiro", "hola2022", "aldinho@gmail.com");
            Periodista p2 = new Periodista("Armando", "Honorio", "chau2022",  "armando@gmail.com");
            AltaPeriodista(p1);
            AltaPeriodista(p2);

            Operador o1 = new Operador("Mateo", "Guerzoni", "mateo123", "mateo_guerzoni@hotmail.com", new DateTime(2021, 12, 26));
            Operador o2 = new Operador("Armando", "Honorio", "mando123", "mando1990@hotmail.com", new DateTime(2021, 12, 31));
            AltaOperador(o1);
            AltaOperador(o2);
          ...
          //metodo de alta partido
        public void AltaPartido(Partido unPartido)
        {
            try
            {
                unPartido.EsValido();
                _partidos.Add(unPartido);
        ...
        //metodo para mostrar pais
        public void MostrarPaises()
        {
            foreach (Pais unPais in this._paises)
            {
                Console.WriteLine("Paises que disputan la copa: " + unPais.Nombre);
            }
        }
        
        //Se da de alta al pais en la lista de paises
        ...
        //metodo para devolver un usuario segun una contraseña dada
        public Usuario GetUsuario(string password)
        {
        ...
        //metodo para devolver un periodista por un email
        ...
        //Ingreso momentaneo los usuarios
        ...
        //Registro de Periodista en Login
        ...
        //Hacemos el metodo de altaJugador para poder llamar desde la precarga
        public void AltaJugador(Jugador unJugador)
        {
            try
            {
                unJugador.EsValido();
                this._jugadores.Add(unJugador);
            }catch (Exception e)
            {
                throw e;
            }
        ...
        //Empezamos con la precarga de Jugadores
        ...
        public void PrecargaPartidos()
        {
            //-------------------Partidos Grupo A--------------------------------------------//
            Seleccion Catar = ObtenerSeleccion("Catar");
            Seleccion Ecuador = ObtenerSeleccion("Ecuador");
            Seleccion Senegal = ObtenerSeleccion("Senegal");
            Seleccion PaisesBajos = ObtenerSeleccion("Países Bajos");
           
            
            //Comienza partido 1
            FaseDeGrupo partido1 = new FaseDeGrupo("A",new DateTime(2022, 11 ,21), "No finalizado", "Pendiente");
            AltaPartido(partido1);

            partido1.AgregarSeleccionesVs(Catar, Ecuador);
            //while (partido1.Estado != "No Finalizado")
            //{
                Incidencia unaIncidencia1 = new Incidencia("Gol", ObtenerJugadorPorId(33), 86);
                Incidencia unaIncidencia2 = new Incidencia("Amarilla", ObtenerJugadorPorId(456), 88);
                Incidencia unaIncidencia3 = new Incidencia("Gol", ObtenerJugadorPorId(472), 90);
                partido1.AgregarUnaIncidencia(unaIncidencia1);
                partido1.AgregarUnaIncidencia(unaIncidencia2);
                partido1.AgregarUnaIncidencia(unaIncidencia3);
            //}
            //Comienza partido 2
            FaseDeGrupo partido2 = new FaseDeGrupo("A", new DateTime(2022, 11, 21), "No finalizado", "Pendiente");
            AltaPartido(partido2);
            partido2.AgregarSeleccionesVs(Senegal, PaisesBajos);

            
            //while (partido2.Estado != "No Finalizado")
            //{
                Incidencia unaIncidencia4 = new Incidencia("Gol", ObtenerJugadorPorId(340), 12);
                Incidencia unaIncidencia5 = new Incidencia("Gol", ObtenerJugadorPorId(332), 45);
                Incidencia unaIncidencia6 = new Incidencia("Amarilla", ObtenerJugadorPorId(574), 55);
                Incidencia unaIncidencia7 = new Incidencia("Gol", ObtenerJugadorPorId(572), 79);
                Incidencia unaIncidencia8 = new Incidencia("Gol", ObtenerJugadorPorId(322), 86);
                Incidencia unaIncidencia9 = new Incidencia("Amarilla", ObtenerJugadorPorId(578), 92);
                partido2.AgregarUnaIncidencia(unaIncidencia4);
                partido2.AgregarUnaIncidencia(unaIncidencia5);
                partido2.AgregarUnaIncidencia(unaIncidencia6);
                partido2.AgregarUnaIncidencia(unaIncidencia7);
                partido2.AgregarUnaIncidencia(unaIncidencia8);
                partido2.AgregarUnaIncidencia(unaIncidencia9);
            //}
            //Comienza partido 3
            FaseDeGrupo partido3 = new FaseDeGrupo("A", new DateTime(2022, 11, 21), "No finalizado", "Pendiente");
            AltaPartido(partido3);
            partido3.AgregarSeleccionesVs(Senegal, Catar);

            //while (partido3.Estado != "No Finalizado")
            //{
                Incidencia unaIncidencia10 = new Incidencia("Amarilla", ObtenerJugadorPorId(39), 5);
                Incidencia unaIncidencia11 = new Incidencia("Gol", ObtenerJugadorPorId(587), 10);
                Incidencia unaIncidencia12 = new Incidencia("Roja", ObtenerJugadorPorId(39), 34);
                Incidencia unaIncidencia13 = new Incidencia("Gol", ObtenerJugadorPorId(580), 60);
                Incidencia unaIncidencia14 = new Incidencia("Gol", ObtenerJugadorPorId(572), 65);
                Incidencia unaIncidencia15 = new Incidencia("Amarilla", ObtenerJugadorPorId(34), 89);
                partido3.AgregarUnaIncidencia(unaIncidencia10);
                partido3.AgregarUnaIncidencia(unaIncidencia11);
                partido3.AgregarUnaIncidencia(unaIncidencia12);
                partido3.AgregarUnaIncidencia(unaIncidencia13);
                partido3.AgregarUnaIncidencia(unaIncidencia14);
                partido3.AgregarUnaIncidencia(unaIncidencia15);
            //}

            //Comienza partido 4
            FaseDeGrupo partido4 = new FaseDeGrupo("A", new DateTime(2022, 11, 29), "No finalizado", "Pendiente");
            AltaPartido(partido4);
            partido4.AgregarSeleccionesVs(PaisesBajos , Ecuador);

            //while (partido3.Estado != "No Finalizado")
            //{
                Incidencia unaIncidencia16 = new Incidencia("Gol", ObtenerJugadorPorId(341), 75);
                Incidencia unaIncidencia17 = new Incidencia("Gol", ObtenerJugadorPorId(454), 80);
                Incidencia unaIncidencia18 = new Incidencia("Amarilla", ObtenerJugadorPorId(333), 81);
               
                partido4.AgregarUnaIncidencia(unaIncidencia16);
                partido4.AgregarUnaIncidencia(unaIncidencia17);
                partido4.AgregarUnaIncidencia(unaIncidencia18);
                
            //}


            //Comienza partido 5
            FaseDeGrupo partido5 = new FaseDeGrupo("A", new DateTime(2022, 11, 29), "No finalizado", "Pendiente") ;
            AltaPartido(partido5);
            partido5.AgregarSeleccionesVs(PaisesBajos, Catar);
            
            //while (partido5.Estado != "No Finalizado")
            //{
                Incidencia unaIncidencia19 = new Incidencia("Gol", ObtenerJugadorPorId(341), 22);
                Incidencia unaIncidencia20 = new Incidencia("Gol", ObtenerJugadorPorId(323), 65);
                Incidencia unaIncidencia21 = new Incidencia("Gol", ObtenerJugadorPorId(333), 76);
                partido5.AgregarUnaIncidencia(unaIncidencia19);
                partido5.AgregarUnaIncidencia(unaIncidencia20);
                partido5.AgregarUnaIncidencia(unaIncidencia21);
            //}

            //Comienza partido 6
            FaseDeGrupo partido6 = new FaseDeGrupo("A", new DateTime(2022, 11, 29), "No finalizado", "Pendiente");
            AltaPartido(partido6);
            partido6.AgregarSeleccionesVs(Ecuador, Senegal);
            //while (partido6.Estado != "No Finalizado")
            //{
                Incidencia unaIncidencia22 = new Incidencia("Gol", ObtenerJugadorPorId(467), 15);
                Incidencia unaIncidencia23 = new Incidencia("Gol", ObtenerJugadorPorId(588), 37);
                Incidencia unaIncidencia24 = new Incidencia("Gol", ObtenerJugadorPorId(588), 55);
                Incidencia unaIncidencia25 = new Incidencia("Gol", ObtenerJugadorPorId(582), 80);
                partido6.AgregarUnaIncidencia(unaIncidencia22);
                partido6.AgregarUnaIncidencia(unaIncidencia23);
                partido6.AgregarUnaIncidencia(unaIncidencia24);
                partido6.AgregarUnaIncidencia(unaIncidencia25);
            //}

            //-------------------Partidos Grupo H--------------------------------------------//
            Seleccion Portugal = ObtenerSeleccion("Portugal");
            Seleccion Uruguay = ObtenerSeleccion("Uruguay");
            Seleccion CoreaDelSur = ObtenerSeleccion("Corea del Sur");
            Seleccion Ghana = ObtenerSeleccion("Ghana");

            //Comienza partido 7
            FaseDeGrupo partido7 = new FaseDeGrupo("A", new DateTime(2022, 11, 24), "No finalizado", "Pendiente");
            AltaPartido(partido7);
            partido7.AgregarSeleccionesVs(Uruguay, CoreaDelSur);
            //while (partido7.Estado != "No Finalizado")
            //{
                Incidencia unaIncidencia26 = new Incidencia("Amarilla", ObtenerJugadorPorId(396), 12);
                Incidencia unaIncidencia27 = new Incidencia("Amarilla", ObtenerJugadorPorId(483), 25);
                Incidencia unaIncidencia28 = new Incidencia("Gol", ObtenerJugadorPorId(387), 42);
                Incidencia unaIncidencia29 = new Incidencia("Gol", ObtenerJugadorPorId(494), 58);
                Incidencia unaIncidencia30 = new Incidencia("Gol", ObtenerJugadorPorId(490), 90);
                partido7.AgregarUnaIncidencia(unaIncidencia26);
                partido7.AgregarUnaIncidencia(unaIncidencia27);
                partido7.AgregarUnaIncidencia(unaIncidencia28);
                partido7.AgregarUnaIncidencia(unaIncidencia29);
                partido7.AgregarUnaIncidencia(unaIncidencia30);

            //}
            
            //Comienza partido 8
            FaseDeGrupo partido8 = new FaseDeGrupo("A", new DateTime(2022, 11, 24), "No finalizado", "Pendiente");
            AltaPartido(partido8);
            partido8.AgregarSeleccionesVs(Portugal, Ghana);
            //while (partido8.Estado != "No Finalizado")
            //{
                Incidencia unaIncidencia31 = new Incidencia("Gol", ObtenerJugadorPorId(541), 5);
                Incidencia unaIncidencia32 = new Incidencia("Gol", ObtenerJugadorPorId(611), 7);
                Incidencia unaIncidencia33 = new Incidencia("Amarilla", ObtenerJugadorPorId(536), 30);
                Incidencia unaIncidencia34 = new Incidencia("Gol", ObtenerJugadorPorId(614), 40);
                Incidencia unaIncidencia35 = new Incidencia("Gol", ObtenerJugadorPorId(541), 64);
                Incidencia unaIncidencia36 = new Incidencia("Gol", ObtenerJugadorPorId(617), 78);
                Incidencia unaIncidencia37 = new Incidencia("Roja", ObtenerJugadorPorId(600), 89);
                partido8.AgregarUnaIncidencia(unaIncidencia31);
                partido8.AgregarUnaIncidencia(unaIncidencia32);
                partido8.AgregarUnaIncidencia(unaIncidencia33);
                partido8.AgregarUnaIncidencia(unaIncidencia34);
                partido8.AgregarUnaIncidencia(unaIncidencia35);
                partido8.AgregarUnaIncidencia(unaIncidencia36);
                partido8.AgregarUnaIncidencia(unaIncidencia37);
            //}

            //Comienza partido 9
            FaseDeGrupo partido9 = new FaseDeGrupo("A", new DateTime(2022, 11, 28), "No finalizado", "Pendiente");
            AltaPartido(partido9);
            partido9.AgregarSeleccionesVs(Portugal, Uruguay);

            //while (partido9.Estado != "No Finalizado")
            //{
                Incidencia unaIncidencia38 = new Incidencia("Gol", ObtenerJugadorPorId(614), 55);
                Incidencia unaIncidencia39 = new Incidencia("Gol", ObtenerJugadorPorId(490), 75);
                Incidencia unaIncidencia40 = new Incidencia("Roja", ObtenerJugadorPorId(483), 80);
                Incidencia unaIncidencia41 = new Incidencia("Amarilla", ObtenerJugadorPorId(603), 84);
                Incidencia unaIncidencia42 = new Incidencia("Amarilla", ObtenerJugadorPorId(617), 89);
                Incidencia unaIncidencia43 = new Incidencia("Gol", ObtenerJugadorPorId(499), 90);
                partido9.AgregarUnaIncidencia(unaIncidencia38);
                partido9.AgregarUnaIncidencia(unaIncidencia39);
                partido9.AgregarUnaIncidencia(unaIncidencia40);
                partido9.AgregarUnaIncidencia(unaIncidencia41);
                partido9.AgregarUnaIncidencia(unaIncidencia42);
                partido9.AgregarUnaIncidencia(unaIncidencia43);
            //}

            //Comienza partido 10

            FaseDeGrupo partido10 = new FaseDeGrupo("A", new DateTime(2022, 11, 28), "No finalizado", "Pendiente");
            AltaPartido(partido10);
            partido10.AgregarSeleccionesVs(CoreaDelSur, Ghana);

            //while (partido10.Estado != "No Finalizado")
            //{
                Incidencia unaIncidencia44 = new Incidencia("Amarilla", ObtenerJugadorPorId(378), 23);
                Incidencia unaIncidencia45 = new Incidencia("Amarilla", ObtenerJugadorPorId(557), 24);
                Incidencia unaIncidencia46 = new Incidencia("Gol", ObtenerJugadorPorId(565), 34);
                Incidencia unaIncidencia47 = new Incidencia("Gol", ObtenerJugadorPorId(388), 67);
                Incidencia unaIncidencia48 = new Incidencia("Gol", ObtenerJugadorPorId(398), 77);
                Incidencia unaIncidencia49 = new Incidencia("Roja", ObtenerJugadorPorId(561), 78);
                partido10.AgregarUnaIncidencia(unaIncidencia44);
                partido10.AgregarUnaIncidencia(unaIncidencia45);
                partido10.AgregarUnaIncidencia(unaIncidencia46);
                partido10.AgregarUnaIncidencia(unaIncidencia47);
                partido10.AgregarUnaIncidencia(unaIncidencia48);
                partido10.AgregarUnaIncidencia(unaIncidencia49);
            //}
           
            //Comienza partido 11

            FaseDeGrupo partido11= new FaseDeGrupo("A", new DateTime(2022, 11, 28), "No finalizado", "Pendiente");
            AltaPartido(partido11);
            partido11.AgregarSeleccionesVs(CoreaDelSur, Portugal);

            //while (partido11.Estado != "No Finalizado")
            //{
                Incidencia unaIncidencia50 = new Incidencia("Gol", ObtenerJugadorPorId(398), 20);
                Incidencia unaIncidencia51 = new Incidencia("Gol", ObtenerJugadorPorId(611), 45);
                Incidencia unaIncidencia52 = new Incidencia("Amarilla", ObtenerJugadorPorId(602), 49);
                Incidencia unaIncidencia53 = new Incidencia("Roja", ObtenerJugadorPorId(388), 60);
                Incidencia unaIncidencia54 = new Incidencia("Amarilla", ObtenerJugadorPorId(378), 61);
                Incidencia unaIncidencia55 = new Incidencia("Gol", ObtenerJugadorPorId(602), 81);
                partido11.AgregarUnaIncidencia(unaIncidencia50);
                partido11.AgregarUnaIncidencia(unaIncidencia51);
                partido11.AgregarUnaIncidencia(unaIncidencia52);
                partido11.AgregarUnaIncidencia(unaIncidencia53);
                partido11.AgregarUnaIncidencia(unaIncidencia54);
                partido11.AgregarUnaIncidencia(unaIncidencia55);
            //}

            //Comienza partido 12

            FaseDeGrupo partido12 = new FaseDeGrupo("A", new DateTime(2022, 11, 28), "No finalizado", "Pendiente");
            AltaPartido(partido12);
            partido12.AgregarSeleccionesVs(Uruguay, Ghana);

            //while (partido12.Estado != "No Finalizado")
            //{
                Incidencia unaIncidencia56 = new Incidencia("Amarilla", ObtenerJugadorPorId(482), 1);
                Incidencia unaIncidencia57 = new Incidencia("Amarilla", ObtenerJugadorPorId(565), 33);
                Incidencia unaIncidencia58 = new Incidencia("Roja", ObtenerJugadorPorId(495), 78);
                Incidencia unaIncidencia59 = new Incidencia("Roja", ObtenerJugadorPorId(557), 79);
                Incidencia unaIncidencia60 = new Incidencia("Roja", ObtenerJugadorPorId(498), 80);
                Incidencia unaIncidencia61 = new Incidencia("Gol", ObtenerJugadorPorId(506), 92);
                partido12.AgregarUnaIncidencia(unaIncidencia56);
                partido12.AgregarUnaIncidencia(unaIncidencia57);
                partido12.AgregarUnaIncidencia(unaIncidencia58);
                partido12.AgregarUnaIncidencia(unaIncidencia59);
                partido12.AgregarUnaIncidencia(unaIncidencia60);
                partido12.AgregarUnaIncidencia(unaIncidencia61);
            //}

            //---------------------------------Fase Eliminatoria---------------------------------//
            FaseEliminatoria partido13 = new FaseEliminatoria( new DateTime(2022, 11, 29), "No finalizada", "Pendiente", "Octavos",false,false);
            AltaPartido(partido13);
            partido13.AgregarSeleccionesVs(Uruguay, Senegal);

            Incidencia unaIndidencia62 = new Incidencia("Gol", ObtenerJugadorPorId(490), 22);
            Incidencia unaIndidencia63 = new Incidencia("Amarilla", ObtenerJugadorPorId(490), 40);
            Incidencia unaIndidencia64 = new Incidencia("Gol", ObtenerJugadorPorId(506), 55);
            Incidencia unaIndidencia65 = new Incidencia("Roja", ObtenerJugadorPorId(572),65);
            Incidencia unaIndidencia66 = new Incidencia("Gol", ObtenerJugadorPorId(490), 82);
            partido13.AgregarUnaIncidencia(unaIndidencia62);
            partido13.AgregarUnaIncidencia(unaIndidencia63);
            partido13.AgregarUnaIncidencia(unaIndidencia64);
            partido13.AgregarUnaIncidencia(unaIndidencia65);
            partido13.AgregarUnaIncidencia(unaIndidencia66);

            FaseEliminatoria partido14 = new FaseEliminatoria( new DateTime(2022, 11, 30), "No finalizada", "Pendiente","Octavos",true,true);
            AltaPartido(partido14);
            partido14.AgregarSeleccionesVs(PaisesBajos , Portugal);

            Incidencia unaIndidencia67 = new Incidencia("Gol", ObtenerJugadorPorId(332), 22);
            Incidencia unaIndidencia68 = new Incidencia("Amarilla", ObtenerJugadorPorId(330), 40);
            Incidencia unaIndidencia69 = new Incidencia("Gol", ObtenerJugadorPorId(340), 55);
            Incidencia unaIndidencia70 = new Incidencia("Amarilla", ObtenerJugadorPorId(328), 65);
            Incidencia unaIndidencia71 = new Incidencia("Amarilla", ObtenerJugadorPorId(617), 70);
            Incidencia unaIndidencia72 = new Incidencia("Gol", ObtenerJugadorPorId(611), 71);
            Incidencia unaIndidencia73 = new Incidencia("Gol", ObtenerJugadorPorId(610), 74);
            Incidencia unaIndidencia74 = new Incidencia("Gol", ObtenerJugadorPorId(611), 80);
            Incidencia unaIndidencia75 = new Incidencia("Gol", ObtenerJugadorPorId(332), 85);
            Incidencia unaIndidencia76 = new Incidencia("Gol", ObtenerJugadorPorId(610), -1);
            Incidencia unaIndidencia77 = new Incidencia("Gol", ObtenerJugadorPorId(611), -1);
            Incidencia unaIndidencia78 = new Incidencia("Gol", ObtenerJugadorPorId(332), -1);
            Incidencia unaIndidencia79 = new Incidencia("Gol", ObtenerJugadorPorId(617), -1);
            partido14.AgregarUnaIncidencia(unaIndidencia67);
            partido14.AgregarUnaIncidencia(unaIndidencia68);
            partido14.AgregarUnaIncidencia(unaIndidencia69);
            partido14.AgregarUnaIncidencia(unaIndidencia70);
            partido14.AgregarUnaIncidencia(unaIndidencia71);
            partido14.AgregarUnaIncidencia(unaIndidencia72);
            partido14.AgregarUnaIncidencia(unaIndidencia73);
            partido14.AgregarUnaIncidencia(unaIndidencia74);
            partido14.AgregarUnaIncidencia(unaIndidencia75);
            partido14.AgregarUnaIncidencia(unaIndidencia76);
            partido14.AgregarUnaIncidencia(unaIndidencia77);
            partido14.AgregarUnaIncidencia(unaIndidencia78);
            partido14.AgregarUnaIncidencia(unaIndidencia79);

            FaseEliminatoria partido15 = new FaseEliminatoria(new DateTime(2022, 12, 01), "No finalizada", "Pendiente", "Cuartos", true, false);
            AltaPartido(partido15);
            partido15.AgregarSeleccionesVs(Uruguay, Portugal);

            Incidencia unaIndidencia80 = new Incidencia("Roja", ObtenerJugadorPorId(483), 5);
            Incidencia unaIndidencia81 = new Incidencia("Amarilla", ObtenerJugadorPorId(485), 25);
            Incidencia unaIndidencia82 = new Incidencia("Amarilla", ObtenerJugadorPorId(610), 90);
            Incidencia unaIndidencia83 = new Incidencia("Gol", ObtenerJugadorPorId(490), 120);
            partido15.AgregarUnaIncidencia(unaIndidencia80);
            partido15.AgregarUnaIncidencia(unaIndidencia81);
            partido15.AgregarUnaIncidencia(unaIndidencia82);
            partido15.AgregarUnaIncidencia(unaIndidencia83);

            FinalizarPartido(partido8.Id);
            FinalizarPartido(partido9.Id);
